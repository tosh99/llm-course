...
const TS_CODE = `// ── Mutual Information ───────────────────────────────────────────
// I(X; Y) = H(X) - H(X|Y)  — implemented from the joint distribution
type JointDist = number[][];

function entropy(p: number[]): number {
  return p.filter(pi => pi > 0).reduce((h, pi) => h - pi * Math.log2(pi), 0);
}

function mutualInformation(p_xy: JointDist): number {
  const rows = p_xy.length, cols = p_xy[0].length;
  const p_x: number[]  = Array.from({ length: rows }, (_, i) => p_xy[i].reduce((s, v) => s + v, 0));
  const p_y: number[]  = Array.from({ length: cols }, (_, j) => p_xy.reduce((s, row) => s + row[j], 0));

  let mi = 0;
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      const pij = p_xy[i][j];
      if (pij > 0 && p_x[i] > 0 && p_y[j] > 0) {
...
// Correlated joint distribution: X and Y are positively correlated
const correlated: JointDist = [
  [0.35, 0.05, 0.02],   // X=0: Y=0,1,2
  [0.05, 0.25, 0.05],   // X=1: Y=0,1,2
  [0.02, 0.05, 0.21],   // X=2: Y=0,1,2
];

const mi = mutualInformation(correlated);
console.log("Mutual information I(X;Y):", mi.toFixed(4), "bits");
// High value → X and Y share a lot of information

// Independent version: product of marginals (should give MI ≈ 0)
const rows = correlated.length, cols = correlated[0].length;
const p_x = Array.from({ length: rows }, (_, i) => correlated[i].reduce((s, v) => s + v, 0));
const p_y = Array.from({ length: cols }, (_, j) => correlated.reduce((s, row) => s + row[j], 0));
const independent: JointDist = p_x.map(pi => p_y.map(pj => pi * pj));

const mi_indep = mutualInformation(independent);
console.log("MI for independent vars:", mi_indep.toFixed(6), "bits ≈ 0 ✓");

// ── Entropy from joint ──────────────────────────────────────────
function jointEntropy(p_xy: JointDist): number {
  const flat = p_xy.flat().filter(pij => pij > 0);
  return -flat.reduce((h, pij) => h + pij * Math.log2(pij), 0);
}

const H_x = entropy(p_x);
const H_y = entropy(p_y);
const H_xy = jointEntropy(correlated);
const mi_from_entropy = H_x + H_y - H_xy;
console.log("\\nI(X;Y) via entropies:", mi_from_entropy.toFixed(4), "bits ✓");

// ── InfoNCE loss ─────────────────────────────────────────────────
function infoNCE(
  positiveSims: number[],   // similarity to positive pair for each sample
  temperature = 0.1
): number {
  const n = positiveSims.length;
  let total = 0;

  for (let i = 0; i < n; i++) {
    // For sample i: numerator = exp(sim_i_positive / tau)
    // Denominator = exp(sim_i_positive / tau) + exp(sim_i_negative / tau)
    // We only have one positive and one negative per sample in this simplified version
    const pos = Math.exp(positiveSims[i] / temperature);
    const neg = Math.exp((-0.1) / temperature); // assumed negative similarity
    total += Math.log(pos / (pos + neg) + 1e-10);
  }

  return -total / n;
}

// Simulate: positive pairs have high similarity (~0.8), negatives low (~-0.1)
const rng = () => 0.8 + 0.05 * (Math.random() - 0.5) * 2;
const positiveSims = Array.from({ length: 256 }, rng);
const loss = infoNCE(positiveSims, 0.1);
console.log("\\nInfoNCE loss:", loss.toFixed(4));
console.log("(Lower loss = higher mutual information between positive pairs)");

// ── Data Processing Inequality ────────────────────────────────────
function discretize(x: number[], nBins: number): number[] {
  const min = Math.min(...x), max = Math.max(...x);
  const edges = Array.from({ length: nBins + 1 }, (_, i) => min + (max - min) * i / nBins);
  return x.map(xi => {
    for (let k = 0; k < nBins; k++) {
      if (xi >= edges[k] && xi <= edges[k + 1]) return k;
    }
    return nBins - 1;
  });
}

function mi_discrete(x: number[], y: number[], nBins = 15): number {
  const xb = discretize(x, nBins);
  const yb = discretize(y, nBins);
  return mutualInformation(
    Array.from({ length: nBins }, (_, i) =>
      Array.from({ length: nBins }, (_, j) =>
        xb.filter((xi, k) => xi === i && yb[k] === j).length / xb.length
      )
    )
  );
}
...
// X → Y → Z chain: Z = Y + noise, Y = X + noise
const X = Array.from({ length: 5000 }, () => (Math.random() - 0.5) * Math.sqrt(12));
const Y = X.map(x => 0.8 * x + 0.6 * (Math.random() - 0.5) * Math.sqrt(12));
const Z = Y.map(y => 0.8 * y + 0.6 * (Math.random() - 0.5) * Math.sqrt(12));

const mi_xy = mi_discrete(X, Y);
const mi_xz = mi_discrete(X, Z);
console.log("\\nData Processing Inequality:");
console.log(\`I(X; Y) = \${mi_xy.toFixed(4)} bits\`);
console.log(\`I(X; Z) = \${mi_xz.toFixed(4)} bits\`);
console.log(\`I(X; Z) ≤ I(X; Y): \${mi_xz <= mi_xy + 0.1 ? "✓" : "✗"}\`);`;

function CodeTab() {
    return (
        <>
            <p>
                Pure TypeScript for empirical mutual information estimation and InfoNCE. These
                implementations show the core algorithmic ideas — discretisation-based estimation
                and the contrastive loss — before reaching for neural network-based estimators
                in high-dimensional ML settings.
            </p>
            <CodeBlock code={TS_CODE} filename="mutual_information.ts" lang="typescript" langLabel="TypeScript" />
            <div className="ch-callout">
                <strong>Key insight:</strong> InfoNCE is not the only way to estimate MI — there
                is also the NWJ estimator (NeurIPS 2019), MINE (ICML 2019), and the Barber-Agakov
                lower bound. Each has different bias-variance properties. InfoNCE dominates in
                practice because it's easy to compute and has good empirical properties, even
                though tighter bounds exist theoretically.
            </div>
        </>
    )
}
