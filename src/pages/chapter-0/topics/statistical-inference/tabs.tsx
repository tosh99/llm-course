...
const TS_CODE = `// ── Point estimation ─────────────────────────────────────────
type Vec = number[];
...
// ── Normal CDF and quantile (approximation) ────────────────────
...
// ── t-distribution approximation (df > 2) ────────────────────
...
function confidenceInterval(data: Vec, alpha = 0.05): [number, number] {
  const n = data.length;
...
// Simulate
const trueMu = 5.0;
const data: Vec = Array.from({ length: 30 }, () =>
  trueMu + 2 * (Math.random() - 0.5) * Math.sqrt(12)
);

const ci = confidenceInterval(data);
console.log(\`95% CI: [\${ci[0].toFixed(3)}, \${ci[1].toFixed(3)}]\`);
console.log(\`True mean \${trueMu} is \${ci[0] <= trueMu && trueMu <= ci[1] ? "IN" : "OUT OF"} the interval\`);

// ── Two-sample t-test (Welch's t-test, unequal variances) ───────
function welchTTest(x: Vec, y: Vec): { t: number; p: number } {
  const n1 = x.length, n2 = y.length;
  const xBar = mean(x), yBar = mean(y);
  const s1Sq = variance(x), s2Sq = variance(y);
  const se = Math.sqrt(s1Sq / n1 + s2Sq / n2);
  const t = (xBar - yBar) / se;
  // Welch-Satterthwaite degrees of freedom
  const v1 = s1Sq / n1, v2 = s2Sq / n2;
...
const modelA = [0.82, 0.85, 0.79, 0.87, 0.84, 0.81, 0.83];
const modelB = [0.78, 0.81, 0.75, 0.79, 0.77];

const { t, p } = welchTTest(modelA, modelB);
console.log(\`\\nWelch's t-test:\`);
console.log(\`Model A: \${mean(modelA).toFixed(4)} ± \${stdDev(modelA).toFixed(4)}\`);
console.log(\`Model B: \${mean(modelB).toFixed(4)} ± \${stdDev(modelB).toFixed(4)}\`);
console.log(\`t = \${t.toFixed(3)}, p = \${p.toFixed(4)} (\${p < 0.05 ? "significant" : "not significant"})\`);

// ── Bootstrap CI (any statistic) ──────────────────────────────
function bootstrapCI(
  ...
  stats.sort((a, b) => a - b);
  return [
    stats[Math.floor(alpha * nBootstrap)],
    stats[Math.floor((1 - alpha) * nBootstrap)],
  ];
}

const expData: Vec = Array.from({ length: 100 }, () => -2 * Math.log(Math.random()));
const ciMean = bootstrapCI(expData, mean);
const ciMedian = bootstrapCI(expData, (x) => {
...
console.log(\`\\nBootstrap 95% CI:\`);
console.log(\`Mean:   [\${ciMean[0].toFixed(3)}, \${ciMean[1].toFixed(3)}]\`);
console.log(\`Median: [\${ciMedian[0].toFixed(3)}, \${ciMedian[1].toFixed(3)}]\`);

// ── Bayesian inference: Normal-Normal conjugate ───────────────
function bayesianNormalPosterior(
  priorMu: number, priorSigma2: number,
  data: Vec,
  likelihoodSigma2: number
): { mu: number; sigma2: number } {
  const n = data.length;
  const xBar = mean(data);
  const priorPrec = 1 / priorSigma2;
  const dataPrec = n / likelihoodSigma2;
  const postPrec = priorPrec + dataPrec;
  const postMu = (priorPrec * priorMu + dataPrec * xBar) / postPrec;
  const postSigma2 = 1 / postPrec;
  return { mu: postMu, sigma2: postSigma2 };
}

const result = bayesianNormalPosterior(5.0, 4.0, data, 4.0);
console.log(\`\\nBayesian Normal-Normal:\`);
console.log(\`Prior: N(5.0, 4.0²)\`);
console.log(\`Posterior: N(\${result.mu.toFixed(3)}, \${Math.sqrt(result.sigma2).toFixed(3)}²)\`);
console.log(\`True mu: \${trueMu}\`);`;

function CodeTab() {
    return (
        <>
            <p>
                Pure TypeScript for confidence intervals, t-tests, bootstrap resampling, and Bayesian
                inference with conjugate priors. These implementations build direct intuition for
                what statistical libraries do under the hood.
            </p>
            <CodeBlock code={TS_CODE} filename="statistical_inference.ts" lang="typescript" langLabel="TypeScript" />
            <div className="ch-callout">
                <strong>Next step:</strong> Use established libraries for production work
                (SciPy in Python, jstat or stats.js in JavaScript). But implementing from scratch
                gives you the deepest understanding — and the ability to debug when things go wrong.
            </div>
        </>
    )
}
