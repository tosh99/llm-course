...
const TS_CODE = `// ── Transformer Encoder Layer — TypeScript ──────────────────────────────────
...
function layerNorm(M: Mat): Mat {
  return M.map(row => {
    const mean = row.reduce((a, b) => a + b, 0) / row.length
    const var_ = row.reduce((a, b) => a + (b - mean) ** 2, 0) / row.length
    return row.map(v => (v - mean) / Math.sqrt(var_ + 1e-6))
  })
}

function reluMat(M: Mat): Mat {
...
class TransformerEncoderLayer {
  ...
  private mha(X: Mat): Mat {
    ...
    for (let h = 0; h < this.numHeads; h++) {
      ...
      const scores = matMul(sliceQ, transpose(sliceK))
      const scaled = scale(scores, 1 / Math.sqrt(dk))
      const attn = softmaxRows(scaled)
      heads.push(matMul(attn, sliceV))
    }
    const concat: Mat = heads[0].map((_, i) => heads.flatMap(head => head[i]))
    return matMul(concat, this.Wo)
  }

  private ffn(X: Mat): Mat {
    const hidden = reluMat(add(matMul(X, this.W1), this.b1.map(b => [b])).map(r => r))
    // b1 broadcast: simplify by assuming column vector applied per row
    const h2 = hidden.map((row, i) => row.map((v, j) => v + this.b1[j]))
    const out = matMul(h2, this.W2)
    return out.map((row, i) => row.map((v, j) => v + this.b2[j]))
  }

  forward(X: Mat): Mat {
    const attnOut = this.mha(layerNorm(X))
    const afterAttn = add(X, attnOut)
    const ffnOut = this.ffn(layerNorm(afterAttn))
...
const dModel = 64, numHeads = 8, dFf = 256
const X = randMat(6, dModel)
const layer = new TransformerEncoderLayer(
  dModel, numHeads, dFf,
  randMat(dModel, dModel), randMat(dModel, dModel),
  randMat(dModel, dModel), randMat(dModel, dModel),
  randMat(dModel, dFf), randVec(dFf),
  randMat(dFf, dModel), randVec(dModel)
)

const out = layer.forward(X)
console.log("Transformer Encoder Layer — TypeScript")
...
function CodeTab() {
    return (
        <>
            <p>
                TypeScript implementation of a Transformer encoder layer with pre-norm,
                multi-head self-attention, and a position-wise feed-forward network.
            </p>
            <CodeBlock code={TS_CODE} filename="transformer_layer.ts" lang="typescript" langLabel="TypeScript" />
        </>
    )
}
