...
const TS_CODE = `// ── LeNet-5 in TypeScript ───────────────────────────────────────────────────

// Simplified LeNet-5 implementation showing the architecture clearly
...
function conv2d(
    ...
    weight: number[][][],
    bias: number[],
    activation: (x: number) => number = (x) => Math.tanh(x)
): Tensor4D {
    const [batch, in_ch, in_h, in_w] = [input.length, input[0].length, input[0][0].length, input[0][0][0].length];
    const [out_ch, _, k_h, k_w] = [weight.length, weight[0].length, weight[0][0].length, weight[0][0][0].length];
    
    const out_h = in_h - k_h + 1;
    const out_w = in_w - k_w + 1;
                    ...
                    for (let ic = 0; ic < in_ch; ic++) {
                        for (let kh = 0; kh < k_h; kh++) {
                            for (let kw = 0; kw < k_w; kw++) {
                                sum += input[b][ic][oh + kh][ow + kw] * weight[oc][ic][kh][kw];
                            }
                        }
                    }
                    output[b][oc][oh][ow] = activation(sum);
                }
            }
        }
    }
...
function avgPool2d(input: Tensor4D, poolSize: number = 2): Tensor4D {
    const [batch, channels, h, w] = [input.length, input[0].length, input[0][0].length, input[0][0][0].length];
    const out_h = Math.floor(h / poolSize);
    const out_w = Math.floor(w / poolSize);
    ...
    for (let b = 0; b < batch; b++) {
        for (let c = 0; c < channels; c++) {
            for (let ph = 0; ph < out_h; ph++) {
                for (let pw = 0; pw < out_w; pw++) {
                    let sum = 0;
                    for (let i = 0; i < poolSize; i++) {
                        for (let j = 0; j < poolSize; j++) {
                            sum += input[b][c][ph * poolSize + i][pw * poolSize + j];
                        }
                    }
                    output[b][c][ph][pw] = sum / (poolSize * poolSize);
                }
            }
        }
    }
...
function flatten(input: Tensor4D): Vec {
...
function linear(input: Vec, weight: Mat, bias: Vec): Vec {
    return weight.map((row, i) => 
        row.reduce((sum, w, j) => sum + w * input[j], 0) + bias[i]
    );
}
...
class LeNet5TS {
    // Weights would be initialized/learned in real training
    C1_weight: number[][][];
    C1_bias: number[];
    C3_weight: number[][][];
    C3_bias: number[];
    ...
    constructor() {
        // C1: 6 filters, 1 channel, 5x5
        this.C1_weight = Array(6).fill(0).map(() => 
            Array(1).fill(0).map(() => 
                Array(5).fill(0).map(() => Array(5).fill(0).map(() => (Math.random() - 0.5) * 0.1))
            )
        );
        this.C1_bias = Array(6).fill(0).map(() => Math.random() * 0.1);

        // C3: 16 filters, 6 channels, 5x5
        this.C3_weight = Array(16).fill(0).map(() => 
            Array(6).fill(0).map(() => 
                Array(5).fill(0).map(() => Array(5).fill(0).map(() => (Math.random() - 0.5) * 0.1))
            )
        );
        this.C3_bias = Array(16).fill(0).map(() => Math.random() * 0.1);

        // C5: 120 filters, 16 channels, 5x5
        this.C5_weight = Array(120).fill(0).map(() => 
            Array(16).fill(0).map(() => 
                Array(5).fill(0).map(() => Array(5).fill(0).map(() => (Math.random() - 0.5) * 0.1))
            )
        );
        this.C5_bias = Array(120).fill(0).map(() => Math.random() * 0.1);
    }

    forward(input: Tensor4D): Vec {
        // C1: 1x32x32 → 6x28x28
        let x = conv2d(input, this.C1_weight, this.C1_bias, Math.tanh);
        
        // S2: 6x28x28 → 6x14x14
        x = avgPool2d(x, 2);
        
        // C3: 6x14x14 → 16x10x10
        x = conv2d(x, this.C3_weight, this.C3_bias, Math.tanh);
        
        // S4: 16x10x10 → 16x5x5
        x = avgPool2d(x, 2);
        
        // C5: 16x5x5 → 120x1x1
        x = conv2d(x, this.C5_weight, this.C5_bias, Math.tanh);
        
        // Flatten: 120x1x1 → 120
        const flat = flatten(x);
        
        // Would continue with FC layers...
        console.log("Output shape after convs:", flat.length, "(should be 120)");
...
// Demo
const lenet = new LeNet5TS();
const dummyInput: Tensor4D = [[Array(32).fill(0).map(() => 
    Array(32).fill(0).map(() => Math.random())
)]];

console.log("Input:", dummyInput[0][0].length, "x", dummyInput[0][0][0].length);
const output = lenet.forward(dummyInput);`;

function CodeTab() {
    return (
        <>
            <p>
                This TypeScript implementation shows each layer of LeNet-5 from scratch.
                Notice the dimension transformations as we go through C1→S2→C3→S4→C5.
            </p>
            <CodeBlock code={TS_CODE} filename="lenet5.ts" lang="typescript" />
        </>
    )
}
