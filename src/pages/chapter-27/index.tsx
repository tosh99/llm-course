...
import "./chapter-27.css"
import { TABS, TOPIC_META, TOPICS } from "./data"
import { RAG_TABS } from "./topics/rag/tabs"
import { DENSE_PASSAGE_RETRIEVAL_TABS } from "./topics/dense-passage-retrieval/tabs"
import { VECTOR_DATABASES_TABS } from "./topics/vector-databases/tabs"
import { CHUNKING_RERANKING_TABS } from "./topics/chunking-reranking/tabs"
import type { TabId, TopicId } from "./types"

const TAB_IDS = TABS.map((t) => t.id)

export function Chapter27Page() {
    ...
    const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        if (dx < 0 && idx < TAB_IDS.length - 1) setActiveTab(TAB_IDS[idx + 1])
            ...
            if (topicIdx < TOPICS.length - 1)
                selectTopic(TOPICS[topicIdx + 1].id)
        ...
        if (dx > 0 && idx > 0) setActiveTab(TAB_IDS[idx - 1])
            ...
            if (topicIdx > 0) selectTopic(TOPICS[topicIdx - 1].id)
                ...
                navigate(`/chapter/${chapterNum - 1}`, {
                    state: { startAtLastTopic: true },
                })
        }
    }

    const tabContent: Record<TopicId, React.ReactNode> = {
        rag: RAG_TABS[activeTab],
        "dense-passage-retrieval": DENSE_PASSAGE_RETRIEVAL_TABS[activeTab],
        "vector-databases": VECTOR_DATABASES_TABS[activeTab],
        "chunking-reranking": CHUNKING_RERANKING_TABS[activeTab],
    }
        ...
        <div className="ch">
            <header className="ch-header">
                <Link to="/" style={{ textDecoration: "none" }}>
                    <span className="ch-header-chapter">Ch. 27</span>
                </Link>
            ...
            <nav className="ch-sidebar" aria-label="Chapter topics">
                ...
                >
                    <span className="ch-nav-icon">
                        {activeTopicEntry?.icon}
                    </span>
                    <span>{topicLabel}</span>
                    <span className="ch-mobile-chevron">▾</span>
                </button>
                <div
                    className={`ch-sidebar-items${mobileNavOpen ? "open" : ""}`}
                >
                    {categories.map((cat, ci) => (
                        <div key={cat}>
                            {ci > 0 && <div className="ch-sidebar-divider" />}
                            <div className="ch-sidebar-label">{cat}</div>
                            {TOPICS.filter((t) => t.category === cat).map(
                                (t) => (
                                    <div
                                        key={t.id}
                    ...
                    {isReady ? (
                        (tabContent[activeTopic] ?? (
                            <div className="ch-coming-soon">
                                Content for <strong>{topicLabel}</strong> is
                                coming soon.
                            </div>
                        ))
                    ) : (
                        <div className="ch-coming-soon">
                            Content for <strong>{topicLabel}</strong> is coming
                            soon.
                        </div>
                    )}
                </article>
